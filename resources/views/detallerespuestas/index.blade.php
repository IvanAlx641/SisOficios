        ...
        <div class="card w-100 position-relative border-0 shadow-sm mb-3">
            <div class="card-body pt-3 pb-2 bg-light d-flex justify-content-between align-items-center border-bottom">
                <div class="d-flex align-items-center">
                    <a href="{{ route('respuestas.index') }}" class="text-guinda text-decoration-none me-3" title="Volver al listado">
                        <i class="ti ti-arrow-back-up fs-3"></i>
                    </a>
                    <h4 class="fw-bold mb-0 text-guinda">Respuesta</h4>
                </div>
                <button class="btn btn-guinda shadow-sm rounded-pill" data-bs-toggle="modal" data-bs-target="#modalAgregarRespuesta">
                    Agregar
                </button>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mt-2">
                                ...
                                @foreach ($oficio->solicitantes as $sol)
                                    <div class="text-dark small"><i class="ti ti-check text-success me-1"></i> {{ $sol->nombre }}</div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('respuestas.store', $oficio->id) }}" method="POST" novalidate>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="date" name="fecha_respuesta" value="{{ old('fecha_respuesta', date('Y-m-d')) }}" class="form-control border-guinda {{ old('form_action') == 'create' && $errors->has('fecha_respuesta') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="numero_oficio_respuesta" value="{{ old('numero_oficio_respuesta') }}" class="form-control border-guinda {{ old('form_action') == 'create' && $errors->has('numero_oficio_respuesta') ? 'is-invalid' : '' }}" placeholder="Ej. 21808000020000L-001/2026">
                                @if(old('form_action') == 'create' && $errors->has('numero_oficio_respuesta'))
                                    <div class="invalid-feedback fw-bold">{{ $errors->first('numero_oficio_respuesta') }}</div>
                                @endif
                            </div>

                            <div class="col-md-6">
                                <label class="form-label fw-bold small">Dirigido a:</label>
                                <select name="dirigido_a_id" class="form-select border-guinda {{ old('form_action') == 'create' && $errors->has('dirigido_a_id') ? 'is-invalid' : '' }}">
                                    <option value="">Seleccione a quién va dirigido...</option>
                                    @foreach ($usuarios as $id => $nombre)
                                        <option value="{{ $id }}" {{ old('dirigido_a_id') == $id ? 'selected' : '' }}>{{ mb_strtoupper($nombre) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="firmado_por_id" class="form-select border-guinda {{ old('form_action') == 'create' && $errors->has('firmado_por_id') ? 'is-invalid' : '' }}">
                                    <option value="">Seleccione quién firma...</option>
                                    @foreach ($usuarios as $id => $nombre)
                                        <option value="{{ $id }}" {{ old('firmado_por_id') == $id ? 'selected' : '' }}>{{ mb_strtoupper($nombre) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="url" name="url_oficio_respuesta" value="{{ old('url_oficio_respuesta') }}" class="form-control border-guinda {{ old('form_action') == 'create' && $errors->has('url_oficio_respuesta') ? 'is-invalid' : '' }}" placeholder="https://...">
                                @if(old('form_action') == 'create' && $errors->has('url_oficio_respuesta'))
                                    <div class="invalid-feedback fw-bold">{{ $errors->first('url_oficio_respuesta') }}</div>
                                @endif
                            </div>

                            <div class="col-md-12 mb-2">
                                <label class="form-label fw-bold small">Descripción de la respuesta:</label>
                                <textarea name="descripción_respuesta_oficio" rows="5" placeholder="Escriba la descripción de la respuesta aquí..." class="form-control border-guinda {{ old('form_action') == 'create' && $errors->has('descripción_respuesta_oficio') ? 'is-invalid' : '' }}">{{ old('descripción_respuesta_oficio') }}</textarea>
                                @if(old('form_action') == 'create' && $errors->has('descripción_respuesta_oficio'))
                                    <div class="invalid-feedback fw-bold">{{ $errors->first('descripción_respuesta_oficio') }}</div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="modal-body p-4 bg-white">
                        ...
                        <form action="{{ route('respuestas.update', $respuesta->id) }}" method="POST" novalidate>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-bold small">Dirigido a:</label>
                                    <select name="dirigido_a_id" class="form-select border-guinda {{ old('form_action') == 'edit_'.$respuesta->id && $errors->has('dirigido_a_id') ? 'is-invalid' : '' }}">
                                        <option value="">Seleccione a quién va dirigido...</option>
                                        @foreach ($usuarios as $id => $nombre)
                                            <option value="{{ $id }}" {{ old('dirigido_a_id', $respuesta->dirigido_a_id) == $id ? 'selected' : '' }}>{{ mb_strtoupper($nombre) }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-bold small">Firmado por:</label>
                                    <select name="firmado_por_id" class="form-select border-guinda {{ old('form_action') == 'edit_'.$respuesta->id && $errors->has('firmado_por_id') ? 'is-invalid' : '' }}">
                                        <option value="">Seleccione quién firma...</option>
                                        @foreach ($usuarios as $id => $nombre)
                                            <option value="{{ $id }}" {{ old('firmado_por_id', $respuesta->firmado_por_id) == $id ? 'selected' : '' }}>{{ mb_strtoupper($nombre) }}</option>
                                        @endforeach
                                    </select>
    ...
    @if($errors->any())
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                setTimeout(function() {
                    // Leemos el form_action que Laravel devolvió
                    let formAction = "{{ old('form_action') }}";
                    let modalId = '';

                    // Determinamos qué modal abrir
                    if (formAction === 'create') {
                        modalId = 'modalAgregarRespuesta';
                    } else if (formAction.startsWith('edit_')) {
