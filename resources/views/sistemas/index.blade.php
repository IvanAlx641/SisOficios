        ...
        <div class="card-body p-4">
            <form action="{{ route('sistema.index') }}" method="GET" id="filterForm">
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        <label class="form-label fw-bold text-guinda small">Nombre del sistema</label>
                        <div class="input-group">
                            <span class="input-group-text bg-white text-guinda border-guinda"><i class="ti ti-search"></i></span>
                            <input type="text" name="nombre_sistema" class="form-control border-guinda" placeholder="Buscar..." value="{{ $request->nombre_sistema }}">
                        </div>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-bold text-guinda small">Sigla</label>
                        <div class="input-group">
                            <span class="input-group-text bg-white text-guinda border-guinda"><i class="ti ti-tag"></i></span>
                            <input type="text" name="sigla_sistema" class="form-control border-guinda" placeholder="Ej. SAM" value="{{ $request->sigla_sistema }}">
                        </div>
                    </div>
                ...
                <div class="row mt-4">
                    <div class="col-md-8 d-flex align-items-center flex-wrap">
                        ...
                        <div class="btn-group me-4 shadow-sm" role="group">
                            <input type="radio" class="btn-check" name="inactivo" value="Todas" id="st_all" onchange="this.form.submit()" {{ ($request->inactivo == 'Todas' || !$request->filled('inactivo')) ? 'checked' : '' }}>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle">
                    <thead class="bg-guinda text-white">
                        <tr>
                            <th class="ps-4 py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Nombre del sistema</h6></th>
                            <th class="text-center py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Sigla</h6></th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    <span class="status-dot {{ $sistema->inactivo == 'X' ? 'dot-inactive' : 'dot-active' }}"></span>
                                    <a href="{{ route('sistema.edit', $sistema->id) }}" class="fs-4 fw-bold mb-0 name-link">
                                        {{ $sistema->nombre_sistema }}
                                    </a>
                                </div>
                            </td>
                            <td class="text-center">
                                <span class="badge badge-guinda-subtle fw-semibold fs-2">{{ $sistema->sigla_sistema }}</span>
                            </td>
                            <td class="text-center">
                                <form action="{{ route('sistema.destroy', $sistema->id) }}" method="POST" onsubmit="return confirm('¿Eliminar sistema?');">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr><td colspan="3" class="text-center py-5 text-muted">No hay resultados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
