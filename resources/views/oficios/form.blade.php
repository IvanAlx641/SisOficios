...
@section('content')
    <div class="card border-0 shadow-sm rounded-3">
        ...
        <div class="card-body p-4 bg-white">
            ...
            <form action="{{ $oficio->exists ? route('oficio.update', $oficio->id) : route('oficio.store') }}" method="POST"
                novalidate>
                @csrf
                @if ($oficio->exists)
                    @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold text-guinda2">Número de oficio:<span
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold text-guinda2">Fecha de recepción:<span
                                class="text-danger">*</span></label>
                        <input type="date" name="fecha_recepcion"
                            class="form-control @error('fecha_recepcion') is-invalid @enderror"
                            value="{{ old('fecha_recepcion', optional($oficio->fecha_recepcion)->format('Y-m-d')) }}"
                            required>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="dirigido_id" id="dirigido_id"
                            class="form-select @error('dirigido_id') is-invalid @enderror" required>
                            <option value="0">Seleccione una unidad...</option>
                            @foreach ($unidades as $id => $nombre)
                                <option value="{{ $id }}"
                                    {{ old('dirigido_id', $oficio->dirigido_id) == $id ? 'selected' : '' }}>
                                    {{ $nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="area_asignada_id" id="area_asignada_id"
                            class="form-select @error('area_asignada_id') is-invalid @enderror" required>
                            <option value="0">Seleccione una unidad...</option>
                            @foreach ($unidades as $id => $nombre)
                                <option value="{{ $id }}"
                                    {{ old('area_asignada_id', $oficio->area_asignada_id) == $id ? 'selected' : '' }}>
                                    {{ $nombre }}
                                </option>
                            @endforeach
                        </select>
                        @error('area_asignada_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="col-md-12">
                        <label class="form-label fw-bold text-guinda2">Descripción del requerimiento:<span
                                class="text-danger">*</span></label>
                        <textarea name="descripción_oficio" class="form-control @error('descripción_oficio') is-invalid @enderror"
                            rows="3" required>{{ old('descripción_oficio', $oficio->descripción_oficio) }}</textarea>
                        @error('descripción_oficio')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="col-md-12">
                        <label class="form-label fw-bold text-guinda2">URL del Oficio (PDF): <span
                                class="text-danger">*</span></label>
                        <input type="url" name="url_oficio"
                            class="form-control @error('url_oficio') is-invalid @enderror" placeholder="https://..."
                            value="{{ old('url_oficio', $oficio->url_oficio) }}" required>
                        @error('url_oficio')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="solicitud_conjunta"
                                id="solicitud_conjunta" value="X"
