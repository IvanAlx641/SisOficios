            ...
            <form id="formBusqueda" method="GET" action="{{ route('buscador.index') }}">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="dirigido_id" id="filtro_dirigido" class="form-select border-guinda text-secondary" onchange="this.form.submit()">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="solicitado_por_id" id="filtro_solicitado" class="form-select border-guinda text-secondary" onchange="this.form.submit()">
                            <option value="Todos">Todos</option>
                    ...
                    <div class="col-md-2 mt-3">
                        ...
                        <select name="estatus" id="filtro_estatus" class="form-select border-guinda text-secondary" onchange="this.form.submit()">
                    ...
                    <div class="col-md-3 mt-3">
                        ...
                        <select name="tipo_requerimiento_id" id="filtro_requerimiento" class="form-select border-guinda text-secondary" onchange="this.form.submit()">
                            <option value="Todos">Todos</option>
                            @foreach($tiposRequerimiento as $id => $nombre)
                                <option value="{{ $id }}" {{ request('tipo_requerimiento_id') == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-1 mt-3 text-end">
                        <button type="submit" class="btn btn-guinda w-100 rounded-pill shadow-sm fw-bold">Buscar</button>
                    </div>
                </div>
            </form>
        </div>
            ...
            <div class="table-responsive" style="min-height: 400px;"> 
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-guinda text-white">
                        <tr>
                            <th class="ps-4 py-3"><h6 class="fs-6 fw-bold mb-0 text-white">Número de oficio</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-6 fw-bold mb-0 text-white">Fecha de<br>recepción</h6></th>
                            <th class="py-3"><h6 class="fs-6 fw-bold mb-0 text-white">Dirigido a</h6></th>
                            <th class="py-3"><h6 class="fs-6 fw-bold mb-0 text-white">Solicitado por</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-6 fw-bold mb-0 text-white">Fecha de<br>turno</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-6 fw-bold mb-0 text-white">Sistema</h6></th>
                            <th class="py-3"><h6 class="fs-6 fw-bold mb-0 text-white">Tipo de requerimiento</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-6 fw-bold mb-0 text-white">Ver<br>oficio</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-6 fw-bold mb-0 text-white">Oficio de<br>respuesta</h6></th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($oficios as $oficio)
                            <tr>
                                <td class="ps-4">
                                    <a href="{{ route('buscador.show', $oficio->id) }}" class="link-hover-guinda fw-bold d-block mb-1">
                                    ...
                                    @endphp
                                    <span class="badge {{ $badgeClass }} rounded-pill px-3">{{ $oficio->estatus ?? 'Atendido' }}</span>
                                </td>
                                
                                <td class="text-center text-muted small">{{ $oficio->fecha_recepcion ? \Carbon\Carbon::parse($oficio->fecha_recepcion)->format('d/m/Y') : '-' }}</td>
                                <td class="small text-muted text-uppercase">{{ $oficio->areaDirigido->nombre_unidad_administrativa ?? 'N/A' }}</td>
                                            ...
                                            <div class="custom-hover-card shadow-lg border rounded bg-white text-start">
                                                ...
                                                <div class="px-3 py-2 text-muted small">
                                                    @foreach($oficio->solicitantes as $sol)
                                                        <div class="text-uppercase mb-1"><i class="ti ti-user me-1 text-guinda"></i>{{ $sol->nombre }}</div>
                                                    @endforeach
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <td class="text-center small text-muted">{{ $oficio->sistema->sigla_sistema ?? 'N/A' }}</td>
                                    ...
                                    @if($oficio->respuestasOficios && $oficio->respuestasOficios->count() > 0)
                                        @php
                                            ...
                                            foreach($oficio->respuestasOficios as $resp) {
                                                ...
                                                $url = $resp->url_oficio_respuesta ? asset('storage/'.$resp->url_oficio_respuesta) : '#';
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-5 text-muted">No se encontraron oficios con esos criterios de búsqueda.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
<style>
    ...
    .link-hover-guinda {
        color: #212529 !important; /* Negro/Gris muy oscuro por defecto */
        text-decoration: none;
        transition: all 0.2s ease;
    }
    .link-hover-guinda:hover {
        color: #9D2449 !important; /* Pasa a Guinda */
