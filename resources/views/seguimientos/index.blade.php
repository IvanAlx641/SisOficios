                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="date" name="fecha_recepcion" class="form-control border-guinda" value="{{ $request->fecha_recepcion }}">
                            <span class="input-group-text bg-white border-guinda">al:</span>
                            <input type="date" name="fecha_recepcion_fin" class="form-control border-guinda" value="{{ $request->fecha_recepcion_fin }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="dirigido_id" id="filtro_dirigido" class="form-select border-guinda" onchange="this.form.submit()">
                            ...
                            @foreach($unidades as $id => $nombre)
                                <option value="{{ $id }}" {{ $request->dirigido_id == $id ? 'selected' : '' }}>{{ $nombre }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="col-md-2">
                        <label class="form-label fw-bold text-guinda2 small mb-2 d-block">Estatus:</label>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="estatus" id="st_todos" value="Todos" onchange="this.form.submit()" {{ ($request->estatus == 'Todos' || !$request->filled('estatus')) ? 'checked' : '' }}>
                            <label class="form-check-label small" for="st_todos">Todos</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="estatus" id="st_turnados" value="Turnado" onchange="this.form.submit()" {{ $request->estatus == 'Turnado' ? 'checked' : '' }}>
                            <label class="form-check-label small" for="st_turnados">Turnados</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="estatus" id="st_concluidos" value="Concluido" onchange="this.form.submit()" {{ $request->estatus == 'Concluido' ? 'checked' : '' }}>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-guinda text-white">
                        <tr>
                            ...
                            <th class="py-3 text-center"><h6 class="fs-4 fw-bold mb-0 text-white">Fecha de<br>recepción</h6></th>
                            ...
                            <th class="py-3 text-center"><h6 class="fs-4 fw-bold mb-0 text-white">Fecha de<br>turno</h6></th>
                            <th class="py-3 text-center"><h6 class="fs-4 fw-bold mb-0 text-white">Sistema</h6></th>
                            <th class="py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Tipo de<br>requerimiento</h6></th>
                            <th class="py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Responsables</h6></th>
                            <th class="text-center py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Ver<br>oficio</h6></th>
                            <th class="text-center py-3"><h6 class="fs-4 fw-bold mb-0 text-white">Concluir</h6></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="small text-muted text-uppercase">{{ $oficio->areaDirigido->nombre_unidad_administrativa ?? '-' }}</td>
                            ...
                            <td class="small text-muted">
                                @if($oficio->solicitantes->count() > 1)
                                    <div class="custom-hover-wrapper position-relative d-inline-block">
                                        <div class="text-uppercase" style="cursor: pointer;">
                                            {{ mb_strtoupper($oficio->solicitantes->first()->nombre) }} <i class="ti ti-arrow-down text-guinda fw-bold ms-1"></i>
                                        </div>
                                ...
                                @elseif($oficio->solicitantes->count() == 1)
                                    <div class="text-uppercase">{{ mb_strtoupper($oficio->solicitantes->first()->nombre) }}</div>
                                @else
                                ...
                                @if($oficio->responsablesOficios->count() > 1)
                                    <div class="custom-hover-wrapper position-relative d-inline-block">
                                        ...
                                        <div class="custom-hover-card shadow-lg border rounded bg-white text-start">
                                            ...
                                            <div class="px-3 py-2 text-muted small fw-normal">
                            ...
                            <td class="text-center">
                                <a href="{{ $oficio->url_oficio }}" target="_blank" class="text-guinda fs-4" title="Ver documento PDF">
                                    <i class="ti ti-eye"></i>
                                </a>
                            </td>
                            ...
                            <td class="text-center">
                                @if($oficio->estatus == 'Turnado')
                                    <button class="btn border-0 bg-transparent text-guinda fs-4" data-bs-toggle="modal" data-bs-target="#modalConcluir{{ $oficio->id }}" title="Concluir oficio">
                                        <i class="ti ti-file-plus"></i>
                                    </button>
                                @else
                                    <button class="btn border-0 bg-transparent text-guinda fs-4" data-bs-toggle="modal" data-bs-target="#modalConcluir{{ $oficio->id }}" title="Editar conclusión">
...
@foreach ($oficios as $oficio)
    ...
    <div class="offcanvas offcanvas-end shadow-lg" tabindex="-1" id="timelineOficio{{ $oficio->id }}" style="width: 450px;">
        <div class="offcanvas-header border-bottom">
            <h5 class="offcanvas-title fw-bold text-guinda">Seguimientos</h5>
            @if($oficio->responsablesOficios->count() > 0)
                <button type="button" class="btn btn-guinda rounded-circle shadow-sm ms-auto me-3 d-flex justify-content-center align-items-center" style="width: 35px; height: 35px;" data-bs-toggle="modal" data-bs-target="#modalAddAvance{{ $oficio->responsablesOficios->first()->id }}" title="Agregar avance">
        ...
        <div class="offcanvas-body p-0 bg-light">
            ...
            <div class="px-3 py-4 bg-white">
                <div class="stepper-wrapper">
                    <div class="stepper-item {{ $nivelActual >= 1 ? 'completed' : '' }} {{ $focoActual == 1 ? 'active' : '' }}" onclick="showTimelineDetails('{{ $oficio->id }}', 'Pendiente', 0)">
                        <div class="step-counter"><i class="ti ti-clipboard-list"></i></div>
                        <div class="step-name">Pendiente</div>
                    </div>
                    <div class="stepper-item {{ $nivelActual >= 2 ? 'completed' : '' }} {{ $focoActual == 2 ? 'active' : '' }}" onclick="showTimelineDetails('{{ $oficio->id }}', 'Desarrollo', 1)">
                        <div class="step-counter"><i class="ti ti-code"></i></div>
                        <div class="step-name">Desarrollo</div>
                    </div>
                    <div class="stepper-item {{ $nivelActual >= 3 ? 'completed' : '' }} {{ $focoActual == 3 ? 'active' : '' }}" onclick="showTimelineDetails('{{ $oficio->id }}', 'Validacion', 2)">
                        <div class="step-counter"><i class="ti ti-shield-check"></i></div>
                        <div class="step-name">Validación</div>
                    </div>
                    <div class="stepper-item {{ $nivelActual >= 4 ? 'completed' : '' }} {{ $focoActual == 4 ? 'active' : '' }}" onclick="showTimelineDetails('{{ $oficio->id }}', 'Publicado', 3)">
            ...
            <div class="p-4 pt-2">
                ...
                <div class="timeline-details-panel" id="panel-Pendiente-{{ $oficio->id }}" style="display: {{ $focoActual == 1 ? 'block' : 'none' }};">
                            ...
                            <div class="card border-0 shadow-sm mb-2 rounded-3">
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <small class="text-muted fw-bold">{{ $seg->fecha_seguimiento->format('d/m/Y') }}</small>
                                    </div>
                ...
                <div class="timeline-details-panel" id="panel-Desarrollo-{{ $oficio->id }}" style="display: {{ $focoActual == 2 ? 'block' : 'none' }};">
                    @if($agrupados['En Desarrollo']->count() > 0)
                        @foreach($agrupados['En Desarrollo'] as $seg)
                            <div class="card border-0 shadow-sm mb-2 rounded-3">
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="badge bg-primary-subtle text-primary rounded-pill">En Desarrollo</span>
                                        <small class="text-muted fw-bold">{{ $seg->fecha_seguimiento->format('d/m/Y') }}</small>
                                    </div>
                                    <p class="mb-0 small text-dark">{{ $seg->observaciones }}</p>
                                </div>
                ...
                <div class="timeline-details-panel" id="panel-Validacion-{{ $oficio->id }}" style="display: {{ $focoActual == 3 ? 'block' : 'none' }};">
                ...
                <div class="timeline-details-panel" id="panel-Publicado-{{ $oficio->id }}" style="display: {{ $focoActual == 4 ? 'block' : 'none' }};">
                    @if($agrupados['Publicado']->count() > 0)
                        @foreach($agrupados['Publicado'] as $seg)
                            <div class="card border-0 shadow-sm mb-2 rounded-3">
                                <div class="card-body p-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="badge bg-success-subtle text-success rounded-pill">Publicado</span>
                                        <small class="text-muted fw-bold">{{ $seg->fecha_seguimiento->format('d/m/Y') }}</small>
                                    </div>
                                    <p class="mb-0 small text-dark">{{ $seg->observaciones }}</p>
                                </div>
                        ...
                        <form action="{{ route('seguimiento.avance.store', $responsableBaseId) }}" method="POST">
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-6">
                                    ...
                                    <input type="date" class="form-control bg-light text-muted border-guinda" value="{{ date('Y-m-d') }}" readonly>
                                </div>
                                <div class="col-6">
                                    <label class="form-label fw-bold text-guinda2 small">Estatus del seguimiento:</label>
            ...
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body p-4">
                    <form action="{{ route('seguimiento.concluir', $oficio->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold text-guinda2 small">Soporte documental:</label>
                            @if($oficio->soporte_documental)
                                <div class="mb-2">
                                    <a href="{{ asset('storage/' . $oficio->soporte_documental) }}" target="_blank" class="badge bg-success text-decoration-none p-2 fs-6">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold text-guinda2 small">Texto propuesta para la respuesta:</label>
                            <div class="border border-guinda border-bottom-0 rounded-top p-2 bg-light d-flex gap-3 text-secondary">
                        ...
                        <div class="form-check mt-4 mb-3 d-flex align-items-center gap-2">
                            <input class="form-check-input mt-0 border-guinda" type="checkbox" name="alcance_otro_oficio" id="checkAlcance{{ $oficio->id }}" value="X" onchange="toggleSelectOficios('{{ $oficio->id }}')" {{ $oficio->alcance_otro_oficio == 'X' ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold text-guinda2 small" for="checkAlcance{{ $oficio->id }}">
                                En alcance a otro oficio:
                            </label>
                        </div>

                        <div id="divSelectOficio{{ $oficio->id }}" class="mb-4 bg-white border border-guinda rounded p-3" style="display: {{ $oficio->alcance_otro_oficio == 'X' ? 'block' : 'none' }};">
                            <label class="form-label text-guinda2 small fw-bold">Buscar y agregar oficios:</label>
                            
                            <div class="d-flex gap-2 mb-3">
                                <div class="flex-grow-1" style="min-width: 0;">
                                    <select id="select_vinculado_{{ $oficio->id }}" class="form-select border-guinda">
                                ...
                                <button type="button" class="btn btn-guinda-light text-nowrap rounded-pill px-3 border border-guinda" onclick="agregarOficioVinculado('{{ $oficio->id }}')">
                                    <i class="ti ti-plus"></i> Agregar
                                </button>
                            </div>
                            
                            <label class="form-label text-guinda2 small fw-bold mb-2">Oficios vinculados a este requerimiento:</label>
                            <div id="lista_vinculados_{{ $oficio->id }}" class="d-flex flex-column gap-2">
                                @foreach($oficio->oficiosVinculados as $vinculado)
                                    <div class="d-flex justify-content-between align-items-center bg-light border border-secondary-subtle rounded px-3 py-2 shadow-sm" id="vinc_{{ $oficio->id }}_{{ $vinculado->id }}">
                                        <span class="text-guinda fw-semibold small"><i class="ti ti-link me-1"></i> {{ $vinculado->numero_oficio }}</span>
                                        <input type="hidden" name="oficios_vinculados[]" value="{{ $vinculado->id }}">
                                        <button type="button" class="btn btn-sm btn-white text-danger border rounded-circle shadow-sm" onclick="removerVinculado('{{ $oficio->id }}', '{{ $vinculado->id }}')">
                                            <i class="ti ti-x"></i>
                                        </button>
                                    </div>
                                @endforeach
                        ...
                        <div class="d-flex align-items-center pt-3 mt-4 border-top">
                            <button type="submit" class="btn btn-guardar-modal rounded-pill px-4 py-2 me-3 shadow-sm">
                                Guardar
                            </button>
                            <button type="button" class="btn-cancelar" data-bs-dismiss="modal">Cancelar</button>
                        </div>
...
<script>
    ...
    function agregarOficioVinculado(oficioId) {
        ...
        div.className = 'd-flex justify-content-between align-items-center bg-light border border-secondary-subtle rounded px-3 py-2 shadow-sm';
